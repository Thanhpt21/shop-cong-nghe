...
const MarqueeBanner = () => {
  const messages = [
    { text: "🔥 HOT DEAL: Giảm tới 30% iPhone 16 Series & Samsung Galaxy S25" },
    { text: "🎧 SIÊU SALE: Tai nghe AirPods Pro 2, Sony WF-1000XM5 giảm sốc" },
    { text: "💻 LAPTOP GIÁ SỐC: MacBook Air M3, Dell XPS, ASUS ROG giảm đến 10 triệu" },
    { text: "⚡ GIAO NHANH 2H: Chỉ nội thành Hà Nội & TP.HCM" },
  ];
  ...
  return (
    <div className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white overflow-hidden">
        ...
        <div className="flex items-center justify-center py-2.5">
          <div className="flex items-center space-x-8 overflow-hidden">
            <div className="flex items-center gap-2 font-bold text-sm whitespace-nowrap">
              <MobileOutlined className="text-yellow-300" />
              <span className="hidden sm:inline">ƯU ĐÃI CÔNG NGHỆ</span>
            </div>
            ...
            <div className="divide-y divide-gray-100">
              {categories.map((category) => (
                <a
                  ...
                  className="flex items-center gap-4 p-4 hover:bg-blue-50 transition-colors duration-200 group/item cursor-pointer block"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-gray-900 group-hover/item:text-blue-600 transition-colors line-clamp-1">
                      {category.name}
                    </div>
                  ...
                  <RightOutlined className="text-gray-400 text-xs group-hover/item:text-blue-600 transition-colors flex-shrink-0" />
                </a>
              ))}
            </div>
          ) : (
            <div className="p-6 text-center text-gray-500 text-sm">Chưa có danh mục</div>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="relative w-full max-w-xl">
      <form onSubmit={handleSearch} className="relative">
        <input
          type="text"
          placeholder="Tìm iPhone 16, MacBook M3, tai nghe Sony, gaming gear..."
          value={searchValue}
          onChange={(e) => setSearchValue(e.target.value)}
          className="w-full h-11 pl-5 pr-12 text-sm bg-white border border-gray-300 rounded-full text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm"
        />
        <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 hover:text-blue-600 transition-colors">
...
const Header = ({ config }: HeaderProps) => {
  ...
  // Trang chủ chỉ active khi đúng ở "/"
  ...
  const CartButton = (
    <Link href="/gio-hang" className="relative flex items-center justify-center w-10 h-10 rounded-lg hover:bg-gray-100 transition-colors">
  ...
  return (
    <>
      ...
      {/* Header trắng + shadow khi scroll */}
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-0">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden lg:flex items-center space-x-10 mx-10">
              {mainMenuItems.map((item) => (
                <div key={item.href} className="relative group">
                  ...
                  {/* Gạch chân chỉ hiện khi active hoặc hover */}
            ...
            <div className="flex items-center space-x-3">
              ...
              <button onClick={toggleSearch} className="lg:hidden flex items-center justify-center w-10 h-10 rounded-lg hover:bg-gray-100 transition-colors">
                {isSearchOpen ? <CloseOutlined className="text-lg text-gray-700" /> : <SearchOutlined className="text-lg text-gray-700" />}
              </button>
                ...
                {isLoggedInUI ? (
                    ...
                    overlay={
                      <Menu className="!rounded-lg !shadow-xl border border-gray-200">
                        <Menu.Item key="account">
                          <Link href="/tai-khoan" className="flex items-center gap-2">Tài khoản</Link>
                        </Menu.Item>
                        {isAdmin && (
                          <Menu.Item key="admin">
                            <Link href="/admin" className="flex items-center gap-2">Quản trị</Link>
                          </Menu.Item>
                        )}
                  ...
                  >
                    <button className="flex items-center justify-center w-10 h-10 rounded-lg hover:bg-gray-100 transition-colors">
                      {currentUser?.avatar ? (
                        <Avatar src={getImageUrl(currentUser.avatar)} size={34} className="ring-2 ring-blue-300" />
                      ) : (
                        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-600 to-cyan-600 flex items-center justify-center text-white font-bold">
              ...
              <button onClick={() => setIsMobileMenuOpen(true)} className="lg:hidden flex items-center justify-center w-10 h-10 rounded-lg hover:bg-gray-100 transition-colors">
          ...
          {/* Mobile Search Bar */}
          <div className={`lg:hidden overflow-hidden transition-all duration-300 ${isSearchOpen ? "max-h-24 opacity-100 pb-4" : "max-h-0 opacity-0"}`}>
      ...
      {/* Mobile Menu Drawer - giữ nguyên như code gốc */}
