  ...
  return (
    <div className="group" style={{ animation: `fadeInUp 0.5s ease-out ${index * 50}ms both` }}>
      <Link href={`/san-pham/${p.slug}`} className="block h-full">
        <Card
          hoverable={false}
          className="bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-blue-200 shadow-sm hover:shadow-xl transition-all duration-300 h-full"
          bodyStyle={{ padding: 0 }}
        >
          {/* Featured Badge - nhỏ xinh, treo góc trên trái */}
          <div className="absolute top-2 left-2 z-20 pointer-events-none">
            <div className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white text-[10px] font-bold px-2 py-1 rounded-md shadow-sm flex items-center gap-1">
              <Star size={10} className="fill-white" />
              <span className="tracking-tight">NỔI BẬT</span>
            </div>
          </div>
          ...
          <div className="relative aspect-square overflow-hidden bg-gray-50">
            ...
            {/* Gift Badge - nếu có, đẩy xuống dưới badge featured */}
            {hasGift && (
              <div className="absolute top-12 right-3 z-10">
                <div className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-md flex items-center gap-1">
          ...
          <div className="p-5 space-y-3">
            <Tooltip title={p.name} placement="top">
              <h3 className="font-semibold text-gray-900 text-sm leading-tight line-clamp-3 group-hover:text-blue-600 transition-colors duration-300">
                {p.name}
              </h3>
            </Tooltip>
