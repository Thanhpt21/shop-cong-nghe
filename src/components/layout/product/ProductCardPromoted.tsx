    ...
    <div className="group" style={{ animation: `fadeInUp 0.5s ease-out ${index * 50}ms both` }}>
      <Link href={`/san-pham/${p.slug}`} className="block h-full">
        <Card
          ...
          bodyStyle={{ padding: 0 }}
        >
          {/* Discount Badge - top-right, chỉ hiện nếu có giảm > 0% */}
          ...
          {/* Gift Badge - top-left, chỉ hiện nếu có quà tặng */}
          {hasGift && (
            <div className="absolute top-2 left-2 z-20 pointer-events-none">
              <div className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white text-[10px] font-bold px-2 py-1 rounded-md shadow-sm flex items-center gap-1">
          ...
          <div className="p-5 space-y-3">
            ...
            <Tooltip title={p.name} placement="top">
              <h3 className="font-semibold text-gray-900 text-sm leading-tight line-clamp-2 group-hover:text-blue-600 transition-colors duration-300">
                {p.name}
              </h3>
            </Tooltip>
